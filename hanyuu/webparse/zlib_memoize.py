...
def zlib_memoize(
    filename: str, key_creator: Callable[..., str], encoding: str = "utf-8"
) -> Callable:
    """Cache with unbounded storage and zlib compression"""

    def wrapper(
        user_function: Callable[..., Awaitable[Optional[str]]]
        ...
        async def wrapped(*args, **kwargs) -> Optional[str]:
            ...
            async with async_session() as session:
                ...
                if item is not None:
                    return (
                        zlib.decompress(item.value).decode(encoding=encoding)
            ...
            async with async_session() as session:
                c_value = (
                    zlib.compress(value.encode(encoding=encoding))
