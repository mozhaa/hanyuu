...
# from hanyuu.database.main.models import QItemDifficulty, QItemSource, QItemSourceTiming, QuizPart
...
# async def process_invalid_records(
#     records: list[tuple[int, str]], entity: type[QItemSource | QuizPart], delete: bool
# ) -> None:
...
#                 if obj is None:
#                     logger.warning(f"Skipping {entity} with id={id_} because it does not exist")
#                     continue
#                 if delete:
#                     logger.info(f"Deleting ({obj}) from database")
#                     await session.delete(obj)
#                 else:
#                     logger.info(f"Unsetting local_fp={fp} from qitem_source with id={id_}")
#                     obj.local_fp = None
#         await session.commit()
#     logger.info(f"Total records {'deleted' if delete else 'unset'}: {n_deleted_records}")
...
#     await process_invalid_records(source_files, QItemSource, False)  # type: ignore
#     delete_unused_files(videos_dir / "sources", [x[1] for x in source_files])
#     await process_invalid_records(quizpart_files, QuizPart, True)  # type: ignore
